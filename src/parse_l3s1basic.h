...
// テープのギャップ
#define CMT_HEADER_GAP "\xff\x01\x3c"

/// パーサークラス
class ParseL3S1Basic : public Parse
{
protected:
	bool mHasCodeFe;	// コード0xfeが入っている

	/// 初期設定
	enum enL3DefaultConfigs {
		eL3DefaultNewLineAscii = 0,	// CR
		eL3DefaultStartAddr = 2
	};
	...
	static const int iStartAddr[eL3StartAddrCount];
...
//	int  OctHexStrToBytes(const wxString &src, wxUint8 *dst, size_t dst_len);
	/// 8/16進文字列のバイト文字列に変換
	int  OctHexStrToBinStr(const wxUint8 *src, size_t src_len, PsSymbol &dst);
	/// 8/16進文字列が範囲内か
...
public:
	ParseL3S1Basic(ParseCollection *collection);
	~ParseL3S1Basic();
...
//	bool ReloadOpenedBinaryData(int type, int mask, const wxString &basic_type);
	...
	/// スタートアドレスのタイトルを返す
	wxString GetStartAddrTitle() const;
//	/// 行番号を返す
...
//	int  ConvUTF8ToAsciiOneLine(const wxString &in_type, size_t row, const wxString &in_line, wxString &out_line, ParseResult *result = NULL);
	...
	const wxChar *GetExportBasicBinaryTapeImageExtension() const;
	/// BASICバイナリテープイメージエクスポート時の拡張子リストを返す
	const wxChar *GetExportBasicBinaryTapeImageExtensions() const;
};

#endif /* _PARSE_L3S1BASIC_H_ */
